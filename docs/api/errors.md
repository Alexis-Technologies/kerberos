...
| `KerberosHookError` | A lifecycle hook threw — `hook` names it, `cause` is the original error. Follows `onError` in the engine, always propagates from the resolver; see [Hooks & events](/guide/hooks#error-contract). |
