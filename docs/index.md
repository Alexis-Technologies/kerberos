...
hero:
  ...
  tagline: Cerbos-style RBAC + ABAC policies, Zanzibar-inspired ReBAC relations and Cerbos-compatible query plans — zero dependencies, no server to deploy, ~25 KB min+gzip.
  image:
    src: /logo-mark.svg
    alt: Kerberos.js
  actions:
...
features:
  - icon: 📭
    ...
    details: No runtime dependencies at all. ~25 KB min+gzip for the main entry, 15 KB for the /relations subpath — nothing extra in your lockfile, nothing extra in the process that makes your access decisions.
