...
3. Otherwise, fall back to `ResourcePolicy` matched by `resource.kind` (rules matched by `roles` / `derivedRoles`, evaluated via `Conditions`/`Variables`/`Constants`/`Outputs`). Conflicts resolve **per principal role**, matching Cerbos ≥ 0.41: `Deny` overrides `Allow` *within* a role, an `Allow` from *any* role wins *across* roles (anti-lockout), and rule order never decides. A rule reached through `derivedRoles` counts for the principal roles in that definition's `parentRoles` — derived roles are not a dimension of their own (`ruleCoversRole` in `ResourcePolicy.js`; the planner mirrors it per role bucket, and `test/PlanParity.test.js` carries multi-role principals specifically to keep the two from drifting).
