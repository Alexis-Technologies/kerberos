...
2. `RolePolicy` entries are a **narrowing filter over the resource layer**, never a granting layer (`#evaluateRolePolicies`), matching Cerbos: allowlists **union** across the principal's roles, and a role with no *applicable* role policy (none targeting `R.kind`) is unrestricted — which disables the filter entirely, so the layer only applies when every principal role is constrained. `parentRoles` still intersect the child's allowed actions with each locally-defined parent role policy (intersection *along the chain*, union *across* roles). A principal-policy override is never narrowed by it.
