...
2. `RolePolicy` entries are a **narrowing filter over the resource layer**, never a granting layer (`#evaluateRolePolicies`), matching Cerbos: allowlists **union** across the principal's roles, and a role with **no role policy at all** is unrestricted — which disables the filter entirely, so the layer only applies when every principal role has one. Holding a role policy constrains that role everywhere: for a kind its rules never target it permits nothing. `parentRoles` still intersect the child's allowed actions with each locally-defined parent role policy (intersection *along the chain*, union *across* roles). A principal-policy override is never narrowed by it.
3. `ResourcePolicy` matched by `resource.kind` is always consulted for the actions the principal policy left open — rules matched by `roles` / `derivedRoles` and evaluated via `Conditions`/`Variables`/`Constants`/`Outputs`, with the role filter from step 2 applied to its result. Conflicts resolve **per principal role**, matching Cerbos ≥ 0.41: `Deny` overrides `Allow` *within* a role, an `Allow` from *any* role wins *across* roles (anti-lockout), and rule order never decides. A rule reached through `derivedRoles` counts for the principal roles in that definition's `parentRoles` — derived roles are not a dimension of their own (`ruleCoversRole` in `ResourcePolicy.js`; the planner mirrors it per role bucket, and `test/PlanParity.test.js` carries multi-role principals specifically to keep the two from drifting).
